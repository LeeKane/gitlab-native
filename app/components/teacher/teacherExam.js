...
export default class TeacherExam extends Component{
  constructor(props)
  {
    super(props);
    this.state = {
  ...
  componentDidMount(){
    ...
    actions.getAllExam(application.username);
  }
  componentWillReceiveProps(props){
    const {repoList} = props;
    const {dataSource} = this.state;
    this.setState({
           dataSource: dataSource.cloneWithRows(repoList.exams.data),
           indicator:false
    })
  }
  renderRow = (exam)=>
  ...
  render(){
    ...
    return (
      ...
      <Toolbar style={styles.navBar} title={'Exam'} onIconPress={drawer.openDrawer} />
      </View>
      {indicator &&
